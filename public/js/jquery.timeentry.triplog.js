...
function TimeEntry() {
	var timeFormat = $('.cur_time_log').attr('time-format');
	var timeFlag = "";
	if (timeFormat=="24") {
		timeFlag = true;
	}else{
	...
	this.regional[''] = { // Default regional settings
		show24Hours: timeFlag, // True to use 24 hour time, false for 12 hour (AM/PM)
...
$.extend(TimeEntry.prototype, {
	...
	_doKeyDown: function(event) {
		if (event.keyCode >= 48 || event.keyCode ==34) { // >= '0'
			return true;
		}
