...
function rent_opt(v) {
    ...
    if (v) {
        ...
        var element = '<div class="form-group" id="ele-' + elem + '">' +
                ...
                '<input name="data[VehicleOffer][rent_opt][' + elem + '][after_day_date]" class="date form-control" placeholder="days" value="0" type="text">' +
                '</div>' +
...
/*$("#panelbody").on('click', ".calendar", function () {
    var ele = $(this);//.parent('div.control-label').closest(".form-group").find('.calwrap').find('.form-control');
    if(ele.attr('defaultviewdate')!== undefined){ele.datepicker({}).datepicker("setDates", new Date(ele.attr('defaultviewdate')));}
    ele.datepicker({}).on("clearDate changeDate", function (event) {
       // $(this).datepicker('hide');
        var datestart = $("#VehicleOfferStartDatetime").val();
        var start = moment(new Date(datestart));
        var end = moment(event.date);
        ele.parent('div.controllabel').closest(".form-group").find('.calwrap').find('.day.form-control').val(end.diff(start, "days") + 1);
        ele.parent('div.controllabel').closest(".form-group").find('.calwrap').find('.date.form-control').val(event.format(0, dateFormat));
...
function qualifyCheckr() {
    ...
    $.post(SITE_URL + 'vehicle_offers/qualify', formdata, function (resp) {
        jQuery.unblockUI();
        if (resp.status) {
...
function calculateFareMatrix() {
    ...
    var formdata = $("#VehicleOfferForm").serialize();
    $.post(SITE_URL + 'vehicle_offers/getVehicleDynamicFareMatrix', formdata, function (obj) {
        jQuery.unblockUI();
...
$(document).ready(function () {
    $("#VehicleOfferForm").submit(function (event) {
        ...
        var formdata = $("#VehicleOfferForm").serialize();
        $.post(SITE_URL + 'vehicle_offers/qualifyIncome', formdata, function (resp) {
            if (!resp.status) {
