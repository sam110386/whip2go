...
$(function() {
    ...
    $.fn.stepy.defaults.nextLabel = 'I Agree <i class="icon-arrow-right14 position-right"></i>';
    ...
    // Initialize wizard
    $("#stepyvalidation").stepy({
        validate: true,
        block: true,
        next: function(index) {
            console.log('index=='+index);
            if (!$("#stepyvalidation").validate(validate)) {
                alert('Going to stedddp: ' + index);
                return false;
            }
        ...
        finish: function(index) {
            if (!$("#stepyvalidation").validate(validate)) {
                alert('Submit canceled due to validation failed.');
                return false
            }
            var params=$("#stepyvalidation").serialize();
            console.log('index=='+index);
            $.post("<?php echo SITE_URL?>eland/elandmob/saveFinalStep",params,function(resp){
    ...
    $('.stepy-step').find('.button-next').addClass('btn bg-pink btn-primary');
