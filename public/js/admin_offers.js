...
function rent_opt(v) {
    ...
    if (v) {
        if (elem === 11) {
            alert("Sorry, you cant add more than 10 records");
            return;
        }
        elem++;
...
function calculateFareMatrix() {
    ...
    $.post(SITE_URL + 'admin/vehicle_offers/getVehicleDynamicFareMatrix', formdata, function (obj) {
        jQuery.unblockUI();
        ...
        var insurance=parseFloat(duration* parseFloat(obj.dayInsurance)).toFixed(2);
...
function qualifyCheckr() {
    ...
    var formdata = $("#VehicleOfferForm").serialize();
    $.post(SITE_URL + 'admin/vehicle_offers/qualify', formdata, function (resp) {
        jQuery.unblockUI();
        if (resp.status) {
...
$(document).ready(function () {
    /*$("#VehicleOfferMiles").change(function(){
        var index=$(this).val();
        var obj=miles_options[index];
        $("#VehicleOfferDays").val(obj.days);
        $("#VehicleOfferDayRent").val(obj.dayRent).removeClass('required').addClass('required');;
        $("#VehicleOfferInsurance").val(obj.dayInsurance);
        $("#VehicleOfferEmf").val(obj.emf);
        ...
        var duration=$("#VehicleOfferDuration").val();
        var rental=parseFloat(duration*obj.dayEmfRent).toFixed(2);
        var dia_fee=parseFloat((rental * parseFloat(calculatedObj.dia_rate)) / 100).toFixed(2);
        var tax=parseFloat((Math.round(rental*100)/100 + Math.round(dia_fee*100)/100) * parseFloat(calculatedObj.tax_rate)).toFixed(2);
        var total_rental=parseFloat(Math.round(rental*100)/100 + Math.round(dia_fee*100)/100 + Math.round(tax*100)/100).toFixed(2);
        var insurance=parseFloat(duration* parseFloat(obj.dayInsurance)).toFixed(2);
        var total_payable_today= parseFloat((Math.round(rental*100)/100+Math.round(insurance*100)/100+Math.round(tax*100)/100+parseFloat(calculatedObj.deposit)+parseFloat(calculatedObj.initial_fee)+Math.round(dia_fee*100)/100)).toFixed(2);;
        
        var program = '';
        program += '<ul>';
        program += '<li><strong>Total Program Cost:</strong> ' + obj.total_program_cost + '</li>';
        program += '<li><strong>Program Fee:</strong> ' + obj.program_fee + '</li>';
        program += '<li><strong>Insurance Cost To Driver:</strong> ' + obj.total_insurance + '</li>';
        program += '<li><strong>Day Rent:</strong> ' + obj.dayRent + '</li>';
        program += '<li><strong>Day Rent With Emf:</strong> ' + obj.dayEmfRent + '</li>';
        program += '<li><strong>Monthly Miles:</strong> ' + obj.month_miles + '</li>';
        program += '<li><strong>Monthly EMF:</strong> ' + obj.month_emf + '</li>';
        ...
        program += '<li><strong>DIA Fee:</strong> ' + dia_fee + '</li>';
        program += '<li><strong>Total Rental:</strong> ' + total_rental+ '</li>';
        program += '<li><strong>Deposit:</strong> ' + parseFloat(calculatedObj.deposit).toFixed(2) + '</li>';
        program += '<li><strong>Insurance:</strong> ' + insurance + '</li>';
        program += '<li><strong>Scheduled Payment:</strong> ' + parseFloat(calculatedObj.initial_fee).toFixed(2) + '</li>';
        program += '<li><strong>Total Due today:</strong> ' + total_payable_today+ '</li></ul>';
        
        $("#calculations").html(program);
    });*/
    
    $("#VehicleOfferFareType").change(function(){
        if($(this).val()=='D'||$(this).val()=='L'){
            $("#VehicleOfferDayRent").removeClass('required');
        }else{
            $("#VehicleOfferDayRent").removeClass('required').addClass('required');
        }
    });
    ...
    $("#VehicleOfferForm").submit(function (event) {
        ...
        var formdata = $("#VehicleOfferForm").serialize();
        $.post(SITE_URL + 'admin/vehicle_offers/qualifyIncome', formdata, function (resp) {
            if (!resp.status) {
