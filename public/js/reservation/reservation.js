function getLicenseScanRequestPopup(booking) {
	...
	$.post(
		SITE_URL + "reservation/pickups/openLicenseScanRequestPopup",
		{ booking: booking },
		function (data) {
...
function saveLicenseScanPopupRequest() {
	if ($("#openLicenseScanRequestPopup").valid()) {
		jQuery.blockUI({
			message:
				'<h1><img src="' +
				SITE_URL +
		...
		jQuery
			.post(
				SITE_URL + "reservation/pickups/saveLicenseScanPopupRequest",
				$("#openLicenseScanRequestPopup").serialize(),
				function (data) {
					alert(data.message);
					if (data.status) {
						$("#myModal").modal("hide");
...
function pickUpUploadPhotoPopup(orderid) {
	...
	$.post(
		SITE_URL + "reservation/pickups/pickUpUploadPhoto",
		{ orderid: orderid },
		function (data) {
			jQuery.unblockUI();
			$("#myModal .modal-content").html(data);
...
function loadPickupChecklistPopup(orderid) {
	...
	jQuery
		.post(
			SITE_URL + "reservation/pickups/loadstatuschecklist",
			{ orderid: orderid },
			function (data) {
				if (!data.status) {
					alert(data.message);
