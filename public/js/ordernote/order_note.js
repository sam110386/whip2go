function getBookingNotes(orderid) {
	...
	$.post(SITE_URL + "admin/order_note/order_notes/loadhistory",{orderid: orderid},function (data) {
...
function loadNewBookingNotesPopup(orderid,parentid) {
	...
	$.post(SITE_URL + "admin/order_note/order_notes/loadnewnotepopup",{ orderid: orderid,parentid:parentid },function (data) {
		$("#myModal .modal-content").html(data);
...
function saveBookingNote() {
	...
	$.post(SITE_URL + "admin/order_note/order_notes/savenote",data,function (data) {
		if (data.status) {
			alert(data.message);
			$("#myModal").modal("hide");
		} else {
