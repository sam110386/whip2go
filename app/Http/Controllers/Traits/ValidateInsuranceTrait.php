...
trait ValidateInsuranceTrait
{
    ...
    protected function filterCoverages(array $coverages, string $vin): array
    {
        foreach ($coverages as $coverage) {
            if (
                strtolower($coverage['type'] ?? '') === 'auto'
                && isset($coverage['details']['vehicle_info']['vin'])
                && strtoupper($vin) == strtoupper($coverage['details']['vehicle_info']['vin'])
            ) {
                return $coverage;
            }
        }
    ...
    protected function validateInsurance(array $policy, array $axleStatus): void
    {
        ...
        if ($policyActive) {
            ...
            AxleStatus::where('id', $axleStatus['id'])->update([
        ...
        if (!$policyActive) {
            ...
            $depositRule = DepositRule::where('vehicle_id', $orderDepositRule->vehicle_id)
            ...
            OrderDepositRule::where('id', $orderDepositRule->id)->update([
