...
trait ReactWebTrait
{
    ...
    public function webFaq(): array
    {
        ...
        return ['status' => 1, 'message' => '', 'result' => ['faq' => $faq, 'categories' => $categories]];
    }

    /**
     * Return static testimonial data.
     */
    public function webTestimonials(): array
    {
        return [
            'status'  => 1,
            'message' => '',
            'result'  => [
                ...
                [
                    'name'      => 'Abraham Diaz',
                    'thumbnail' => 'https://play-lh.googleusercontent.com/a-/AOh14Ghd7O-PSSrenVzr-LsR9fr5QE3izgTJ6KFe9CCvvQ=w96-h96-n-rw',
                    'short'     => 'I give them a rightous 5 star! The vehicle I got was brand new and I made sure to keep it that way. The rental company affiliated with..',
                    'detail'    => 'I give them a rightous 5 star! The vehicle I got was brand new and I made sure to keep it that way. The rental company affiliated with DriveItAway was very courteous and their service team is top notch. I have used them before on several occasions and wasn\'t disappointed on using their services this time around. Lastly the livechat feature is very responsive and the in app customer service team is also exceptional.',
                    'rating'    => 5,
                ],
                [
                    'name'      => 'Stephan Sherard',
                    'thumbnail' => 'https://play-lh.googleusercontent.com/a/AATXAJwuVnNxk6pLEkW6mPVaKy2uJ9LdjGe8llMqoaz2=w96-h96-n-rw-mo',
                    'short'     => 'Great Customer Service and Great Prices for vehicles and very Easy to use app. You guys are Amazing. Thank You So Much.',
                    'detail'    => 'Great Customer Service and Great Prices for vehicles and very Easy to use app. You guys are Amazing. Thank You So Much.',
                    'rating'    => 5,
                ],
            ],
        ];
    ...
    /**
     * Handle contact-us form submission: send email and push to Salesforce.
     */
    public function postContactUs($postData): array
    {
        $data = json_decode($postData, true);
        $dataValues = isset($data['data']) ? $data['data'] : $data;
        ...
        Mail::send('emails.contact_us', $JSONDATA, function ($message) use ($email) {
            $message->from('support@whip2go.com', 'DriveItAway Team')
                ...
                ->to('adam@whip2go.com')
                ->subject('DriveItAway - Contact Us');
        });
    ...
    /**
     * Push lead data to Salesforce WebToLead endpoint.
     */
    private function pushToSalesForce(array $data): void
    {
        $oid = '00D1U000000uO5w';
        $recordType = $data['iam'] ?? '';
        $address = preg_replace('/[^a-zA-Z0-9]/', '', $data['address'] ?? '');

        Http::asForm()->post('https://webto.salesforce.com/servlet/servlet.WebToLead', [
            'encoding'   => 'UTF-8',
            'oid'        => $oid,
    ...
    /**
     * Return static marketing text for vehicle detail pages.
     */
    public function VehicleDetailText(): array
    {
        return [
            'status'  => 1,
            'message' => '',
