...
class VehicleIssueLib
{
    ...
    {
        $vehicles = Vehicle::where('booked', 1)
        ...
        foreach ($vehicles as $vehicle) {
            ...
            CsVehicleIssue::create($issueData);
            $vehicle->update(['total_mileage' => $nextServiceOdometer]);
        }
    }
    ...
    public function createTicketOnBookingComplete(array $opt = []): void
    {
        ...
        $vehicle = Vehicle::where('id', $opt['vehicle_id'])
            ->select('id', 'user_id', 'last_mile', 'total_mileage')
            ->first();
