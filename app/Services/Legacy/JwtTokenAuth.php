...
/**
 * Ported from CakePHP app/Controller/Component/Auth/JwtTokenAuthenticate.php
 * Nearly identical to JwtAuth but uses pepper='mindseye1' and returns
 * expire_at in the refresh() response.
 */
class JwtTokenAuth
{
    ...
    /**
     * Stateless — always returns false.
     */
     ...
     * Returns expire_at in addition to the standard fields (differs from JwtAuth).
     */
    public function refresh(string $token, int $tokenIncreaseTo): array
    {
        try {
            ...
            $expire_at = time() + $tokenIncreaseTo;
            $decoded['iat'] = time();
            $decoded['exp'] = $expire_at;

            $newToken = JWT::encode($decoded, $this->settings['pepper'], 'HS256');
            return ['status' => 1, 'message' => 'Token refreshed', 'auth_token' => $newToken, 'expire_at' => $expire_at];
        } catch (\Exception $e) {
