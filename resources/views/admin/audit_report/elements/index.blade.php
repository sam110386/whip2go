            ...
            @foreach($records as $record)
                <tr>
                    ...
                    <td>
                        &nbsp;
                        @if($record->status == 0)
                            <a href="{{ url('admin/audit_reports/process/' . $record->id) }}" title="Process">
                                <i class="icon-spinner9"></i>
                            </a>
                        @endif
                        @if($record->status)
                            <a href="{{ url('admin/audit_reports/download/' . base64_encode($record->id)) }}">
                                <i class="icon-file-download2"></i>
                            </a>
                        @endif
                        &nbsp;
                        <a href="{{ url('admin/audit_reports/delete/' . base64_encode($record->id)) }}">
