...
@section('content')
<style type="text/css">
    .dropdown-menu > .dropdown-submenu > .dropdown-menu{margin-left: -295px;}
    ...
    <div class="page-header-content">
        ...
        <div class="heading-elements">
            <div class="input-group-btn">
                <a href="{{ url('/admin/inspection_settings') }}" class="btn btn-primary" style="margin-right:20px;">Inspection Schedule Setting</a>
            </div>
            <div class="input-group-btn">
                ...
                <ul class="dropdown-menu pull-right">
                    <li><a href="{{ url('/admin/vehicle_issues/accident') }}">Accident Issue</a></li>
                    <li><a href="{{ url('/admin/vehicle_issues/mechanical') }}">Mechanical Issue</a></li>
                    <li><a href="{{ url('/admin/vehicle_issues/roadside') }}">Roadside Assistance</a></li>
                    <li><a href="{{ url('/admin/vehicle_issues/cleaning') }}">Vehicle Cleaning</a></li>
                    <li><a href="{{ url('/admin/vehicle_issues/maintenance') }}">Maintenance</a></li>
                    <li><a href="{{ url('/admin/vehicle_issues/inspection_scan') }}">Inspection Scan</a></li>
                </ul>
            </div>
        </div>
    </div>
...
<div class="panel">
    <div class="panel-body">
        <form action="{{ url('/admin/vehicle_issues') }}" method="POST" id="frmSearchadmin" name="frmSearchadmin">
            @csrf
            <div class="row pb-10">
                <div class="col-md-12">
                    <div class="col-md-2">
                    ...
                    <div class="col-md-2">
                        <input type="text" name="Search[user_id]" id="SearchUserId" class="formcontrol" value="{{ $user_id }}" style="width:100%;">
                    </div>
                    <div class="col-md-2">
                        <input type="text" name="Search[renter_id]" id="SearchRenterId" class="formcontrol" value="{{ $renter_id }}" style="width:100%;">
                    </div>
                    <div class="col-md-2">
                    ...
                    <div class="col-md-1">
                        <input type="submit" name="Search[ClearFilter]" value="Clear Filter" class="btn btn-warning">
                    </div>
                </div>
            </div>
        </form>
...
<div class="panel">
    <div class="panel-body" id="listing">
        @include('admin.vehicle_issues._index')
    </div>
</div>
...
<script type="text/javascript">
    ...
    jQuery(document).ready(function () {
        ...
        jQuery("#SearchUserId").select2({
            ...
            ajax: {
                url: SITE_URL+"admin/vehicles/ownerautocomplete", dataType: "json", type: "GET",
                data: function (params) { return {term: params}; },
                processResults: function (data) { return {results: jQuery.map(data, function (item) { return {tag: item.tag, id: item.id}; })}; }
            },
            initSelection: function(element, callback) {
                var id = $(element).val();
                if (id !== "") { $.ajax(SITE_URL+"admin/vehicles/ownerautocomplete", {dataType: "json", type:'POST', data:{id:id}}).done(function(data) { callback(data[0]); }); }
            }
        });
        jQuery("#SearchRenterId").select2({
            ...
            initSelection: function(element, callback) {
                var id = $(element).val();
                if (id !== "") { $.ajax(SITE_URL+"admin/bookings/customerautocomplete", {dataType: "json", type:'POST', data:{id:id}}).done(function(data) { callback(data[0]); }); }
            }
        });
    });
    function changemystatus(id, status) {
        ...
        if (con) {
            ...
            $.post(SITE_URL + 'admin/vehicle_issues/changeMyStatus', {id: id, status: status, _token: '{{ csrf_token() }}'}, function (data) {
