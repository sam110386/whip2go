<table width="100%" cellpadding="2" cellspacing="1" border="0" class="table table-responsive">
    ...
    @foreach($PromotionRules as $rule)
        <tr>
            ...
            <td>{{ $rule->created }}</td>
            <td class="action">
                <a href="{{ url('/admin/promo_rules/delete/' . base64_encode($rule->id)) }}"
                   title="Delete"
                   onclick="return confirm('Are you sure you want to delete this promo?');"><i class="icon-trash"></i></a>
                &nbsp;
                <a href="{{ url('/admin/promo_rules/add/' . base64_encode($rule->id)) }}"><i class="icon-pencil"></i></a>
                &nbsp;
                <a href="{{ url('/admin/promo_rules/promousers/' . base64_encode($rule->id)) }}"><i class="icon-user"></i></a>
            </td>
        </tr>
    @endforeach
    <tr>
