...
@section('content')
    <h1>Account General Setting</h1>

    @if (session('success'))
        <p style="color:#1b5e20;">{{ session('success') }}</p>
    @endif

    <form method="POST" action="/admin/settings/index/{{ base64_encode((string)($userId ?? 0)) }}" id="SettingAdminIndexForm" class="settings-grid">
        <fieldset class="settings-row">
            ...
            <div class="form-group geotab">
                <button type="button" class="btn" onclick="validateGeoTab()">Validate Geotab settings</button>
            </div>
            ...
            <div class="form-group onestepgps">
                <button type="button" class="btn" onclick="validateOneStepGPSKey()">Validate One Step GPS key</button>
            </div>
            @if ($gps === 'geotab' || $pst === 'geotab')
                <div class="form-group geotab">
                    <button type="button" id="syncDeviceWithGeotab" onclick="syncVehicleWithGeotab()">Sync Geotab vehicles</button>
                </div>
            @endif
            @if ($gps === 'onestepgps' || $pst === 'onestepgps')
                <div class="form-group onestepgps">
                    <button type="button" onclick="syncVehicleWithOnestep()">Sync OneStep vehicles</button>
                </div>
            @endif
            @if ($gps === 'autopi' || $pst === 'autopi')
                <div class="form-group autopi">
                    <button type="button" id="sycnAutoPiVehicle" onclick="pullAutoPiVehicle()">Sync AutoPi vehicles</button>
                </div>
            @endif
            ...
            <div class="settings-row flex">
                ...
                <button type="button" onclick="SyncVehicleFinancing()">Update all vehicles</button>
            </div>
            ...
            <div class="settings-row flex">
                ...
                <button type="button" onclick="SyncVehicleAllowedMiles()">Update all vehicles</button>
            </div>
            ...
            @if ($depositTemplate)
                ...
                <div class="settings-row flex">
                    ...
                    <button type="button" onclick="updateFareType('roadside_assistance_included')">Sync</button>
                </div>
                ...
                <div class="settings-row flex">
                    ...
                    <button type="button" onclick="updateFareType('maintenance_included_fee')">Sync</button>
                </div>
            @endif
            ...
            <div class="settings-row flex" style="align-items:flex-start;">
                ...
                <button type="button" onclick="SyncVehicleDefaultAddress()">Sync all vehicles</button>
            </div>
        </fieldset>
                    ...
                    <div class="settings-row" id="ele-{{ $k }}" style="border-top:1px solid #eee; padding-top:10px;">
                        ...
                        <div style="display:flex; gap:8px; flex-wrap:wrap; align-items:center;">
                            ...
                            @if ($k === array_key_first($locationRows))
                                <button type="button" onclick="address_more(true)">+</button>
                                <button type="button" onclick="SyncVehicleAddress()">Update all vehicles</button>
                            @else
                                <button type="button" onclick="address_more(false)">−</button>
                            @endif
                        </div>
                    </div>
                @endforeach
        ...
        function address_more(v) {
            ...
            if (v) {
                ...
                var element = '<div class="settings-row" id="ele-' + elem + '" style="border-top:1px solid #eee;padding-top:10px;">' +
                    ...
                    '<button type="button" onclick="address_more(false)">−</button></div></div>';
