...
@php
    $title ??= 'Adjust Dealer EMF Transfer';
    $csorder ??= collect();
@endphp
...
@section('content')
    ...
    <div class="panel">
        ...
        <div class="row">
            @if($csorder)
                <form id="adjustemf" class="form-horizontal">
                    @csrf
                        ...
                        <div class="panel-body">
                            ...
                            <fieldset class="content-group">
                                <div class="form-group">
                                    ...
                                    <div class="col-lg-6">
                                        {{ data_get($csorder, 'extra_mileage_fee', 'N/A') }}
                                    </div>
                                </div>
                                <div class="form-group">
                                    <label class="col-lg-4">
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="col-lg-2">
                                        <button type="button" class="btn btn-primary"
                                            onClick="adjustDealerEmfPart('{{ base64_encode(data_get($csorder, 'id', '')) }}')">
                                            Proceed To Adjust
                                        </button>
                                    </div>
                                    <div class="col-lg-2">
                                        <button type="button" class="btn btn-danger btn-ladda btn-ladda-progress"
                                            onClick="emfReversetotal('{{ base64_encode(data_get($csorder, 'id', '')) }}')">
                                            Reverse Total
                                        </button>
                                    </div>
                                </div>
...
@push('scripts')
    ...
    <script type="text/javascript">
        ...
        function adjustDealerEmfPart(orderid) {
            if (orderid.length > 0 && $("#adjustemf").valid()) {
                ...
                if (conf) {
                    ...
                    var params = $("#adjustemf").serialize();
                    $.post(SITE_URL + "admin/transactions/adjustDealerEmfPart", params, function (data) {
                        jQuery.unblockUI();
                        swal({
                            title: "",
                            text: data.message,
        ...
        function emfReversetotal(orderid) {
            if (orderid.length > 0) {
                ...
                if (conf) {
                    ...
                    $.post(SITE_URL + "admin/transactions/emfReversetotal", {
                        "orderid": orderid,
                        "_token": "{{ csrf_token() }}"
                    }, function (data) {
                        jQuery.unblockUI();
