...
@section('content')
    ...
    <div class="panel">
        ...
        <div class="row">
            @if($csorder)
                <form id="ReportAdminAdjustdealerrentaltransferForm" class="form-horizontal">
                    @csrf
                                ...
                                <div class="form-group">
                                    ...
                                    <div class="col-lg-2">
                                        <button type="button" class="btn btn-primary"
                                            onClick="adjustDealerRentalPart('{{ base64_encode(data_get($csorder, 'id', ''))}}')">
                                            Proceed To Adjust
                                        </button>
                                    </div>
                                    <div class="col-lg-2">
                                        <button type="button" class="btn btn-danger btn-ladda btn-ladda-progress"
                                            onClick="rentReversetotal('{{ base64_encode(data_get($csorder, 'id', '')) }}')">
                                            Reverse Total
                                        </button>
                                    </div>
                                </div>
...
@push('scripts')
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            $("#ReportAdminAdjustdealerrentaltransferForm").validate();
        });

        function adjustDealerRentalPart(orderid) {
            if (orderid.length > 0 && $("#ReportAdminAdjustdealerrentaltransferForm").valid()) {
                var conf = confirm('Are you sure you want to adjust dealer amount ?');
                if (conf) {
                    jQuery.blockUI({
                        message: '<h1><img src="' + SITE_URL + 'img/select2-spinner.gif" /> Sending...</h1>',
                        css: { 'z-index': '9999' }
                    });
                    var params = $("#ReportAdminAdjustdealerrentaltransferForm").serialize();
                    $.post(SITE_URL + "admin/transactions/adjustDealerRentalPart", params, function (data) {
                        jQuery.unblockUI();
        ...
        function rentReversetotal(orderid) {
            if (orderid.length > 0) {
                var conf = confirm('Are you sure you want to reverse full transfer ?');
                if (conf) {
                    jQuery.blockUI({
                        message: '<h1><img src="' + SITE_URL + 'img/select2-spinner.gif" /> Sending...</h1>',
                        css: { 'z-index': '9999' }
                    });
                    $.post(SITE_URL + "admin/transactions/rentReversetotal", {
                        "orderid": orderid,
                        "_token": "{{ csrf_token() }}"
                    }, function (data) {
                        jQuery.unblockUI();
