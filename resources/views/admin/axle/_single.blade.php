        ...
        <span class="dropdown">
            ...
            <ul class="dropdown-menu dropdown-menu-solid pull-right">
                ...
                @if (empty($record->axle_status) || in_array($axleStatus, [1, 2, 4]))
                    <li><a href="javascript:void(0)" onclick="axlePolicyDisconnect('{{ $record->order_id }}')" title="Disconnect" class="btn btn-danger" target="_blank">Disconnect<i class="icon-trash position-right"></i></a></li>
                @endif
                @if (!empty($record->account_id) && empty($record->policy))
                    <li><a href="javascript:void(0)" onclick="getAxleAccountDetails('{{ $record->order_id }}')" title="Pull Policy Details" class="btn btn-info">Account Details <i class="icon-spinner4 position-right"></i></a></li>
                @endif
                @if (!empty($record->policy) && in_array($axleStatus, [2, 4, 5, 6]))
                    <li><a href="javascript:void(0)" onclick="getAxlePolicyDetails('{{ $record->order_id }}')" title="Pull Policy Details" class="btn btn-info">Policy Details <i class="icon-spinner4 position-right"></i></a></li>
                @endif
                @if (!empty($record->policy))
                    <li><a href="javascript:void(0)" onclick="axlePolicyDetailsPopup('{{ $record->order_id }}')" title="Policy Checklist" class="btn btn-warning">Checklist <i class="icon-pencil7 position-right"></i></a></li>
                @endif
            </ul>
        </span>
    </td>
