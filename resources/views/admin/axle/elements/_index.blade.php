...
<table width="100%" cellpadding="2" cellspacing="1" border="0" class="table table-responsive">
    ...
    <tbody>
        @foreach ($records as $trip)
            @php
                $bg = 'bg-primary';
                if ($trip->axle_status == 1) {
                    $bg = 'bg-info';
                }

                if ($trip->axle_status == 2 || $trip->axle_status == 5) {
                    $bg = 'bg-success';
                }

                if ($trip->axle_status == 3) {
                    $bg = 'bg-danger';
                }
            @endphp
                ...
                <td>
                    <span class="dropdown">
                        ...
                        <ul class="dropdown-menu dropdown-menu-solid pull-right">
                            @if ((empty($trip->axle_status) || in_array($trip->axle_status, [0, 3])) || (empty($trip->account_id) && empty($trip->policy)))
                                <li>
                                    <a href="{{ url('admin/axledocs/connect/' . $trip->order_deposit_rule_id) }}"
                                        title="Connect to Axle" class="btn btn-success" target="_blank">
                                        Connect to Axle
                                        <i class="icon-arrow-resize7 position-right"></i>
                                    </a>
                                </li>
                            @endif
