...
<table class="table table-responsive" style="width:100%;">
    ...
    <tbody>
        @forelse($reportlists as $r)
            ...
            <tr id="tr_{{ $r->id  }}">
                <td>
                    <input type="checkbox" name="select[{{ $r->id }}]" value="{{ $r->id }}" class="select-item" id="select1"
                        style="border:0;">
                </td>
                ...
                <td>
                    @if($r->status == 3)
                        <a href="javascript:void(0)" onclick="reviewimages('{{ base64_encode($r->id) }}')"
                            title="Review Images">
                            <i class='icon-clipboard3'></i>
                        </a>
                    @endif
                    <a href="javascript:void(0)" onclick="return downloadBookingDoc('{{ base64_encode($r->id) }}')"
                        title="Download Doc">
                        <i class=" icon-file-pdf"></i>
                    </a>
                    <a href="javascript:void(0)" onclick="return inspektScanReport('{{ $r->id }}')"
                        title="Inspeck Scan report">
