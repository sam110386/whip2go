    ...
    <form class="form-horizontal">
        ...
        @if (!empty($reportData['geoLocation']))
            <div class="form-group">
                ...
                <div class="col-lg-9">
                    <a target="_blank"
                        href="{{ sprintf('https://www.google.com/maps/search/%s?sa=X&ved=1t:242&ictx=111', implode(',', $reportData['geoLocation'])) }}">
                        {{ implode(',', $reportData['geoLocation']) }}
                    </a>
                </div>
            </div>
        @endif
                    ...
                    <ul>
                        @foreach ($reportData['vehicleReadings'] as $key => $val)
                            <li>
                                ...
                                {!! is_array($val) ? '<code>' . json_encode($val) . '</code>' : e($val) !!}
                            </li>
                        @endforeach
                    </ul>
                </div>
