            ...
            @if (!empty($csorder))
                ...
                <div class="col-lg-6">
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-lg-6">
                            {{ !empty($csorder['CsOrder']['end_datetime']) && strpos($csorder['CsOrder']['end_datetime'], '0000') !== 0 ? \Carbon\Carbon::parse($csorder['CsOrder']['end_datetime'])->timezone($csorder['CsOrder']['timezone'] ?? config('app.timezone'))->format("m/d/Y h:i A") : '--' }}
                        </div>
                    </div>
                    <div class="form-group">
                        <label class="col-lg-6 ">Actual End Time :</label>
                        <div class="col-lg-6">
                            {{ !empty($csorder['CsOrder']['end_timing']) && strpos($csorder['CsOrder']['end_timing'], '0000') !== 0 ? \Carbon\Carbon::parse($csorder['CsOrder']['end_timing'])->timezone($csorder['CsOrder']['timezone'] ?? config('app.timezone'))->format("m/d/Y h:i A") : 'N/A' }}
                        </div>
                    </div>
                    <div class="form-group">
                        <label class="col-lg-6 ">Renew Time :</label>
                        <div class="col-lg-6">
                            {{ !empty($csorder['CsOrder']['created_at']) && strpos($csorder['CsOrder']['created_at'], '0000') !== 0 ? \Carbon\Carbon::parse($csorder['CsOrder']['created_at'])->timezone($csorder['CsOrder']['timezone'] ?? config('app.timezone'))->format("m/d/Y h:i A") : '--' }}
                        </div>
                    </div>
                    <legend>Fees</legend>
                    <div class="form-group">
                        <label class="col-lg-6 ">Initial Fee :</label>
                        <div class="col-lg-6">{{ $csorder['CsOrder']['initial_fee'] ?? 0 }}</div>
                    </div>
                    <div class="form-group">
                        <label class="col-lg-6 ">Initial Fee Tax :</label>
                        <div class="col-lg-6">{{ $csorder['CsOrder']['initial_fee_tax'] ?? 0 }}</div>
                    </div>
                    <div class="form-group">
                        <label class="col-lg-6 ">Initial Fee Discount :</label>
                        <div class="col-lg-6">{{ $csorder['CsOrder']['initial_discount'] ?? 0 }}</div>
                    </div>
                    <div class="form-group heading-divided-updown text-bold">
                        <label class="col-lg-6 ">Total Initial Fee:</label>
                        <div class="col-lg-6">
                            @php $initlafee = sprintf('%0.2f', (($csorder['CsOrder']['initial_fee'] ?? 0) + ($csorder['CsOrder']['initial_fee_tax'] ?? 0))); @endphp
                            {{ $initlafee }}
                        </div>
                    </div>
                    <!-- Rent -->
                    <div class="form-group">
                        ...
                        <div class="col-lg-6">{{ ($csorder['CsOrder']['rent'] ?? 0) + ($csorder['CsOrder']['discount'] ?? 0) }}</div>
                    </div>
                    <div class="form-group">
                        <label class="col-lg-6 ">Discount :</label>
                        <div class="col-lg-6">{{ $csorder['CsOrder']['discount'] ?? 0 }}</div>
                    </div>
                    <div class="form-group">
                        <label class="col-lg-6">Extra Mileage Fee :</label>
                        <div class="col-lg-6">{{ $csorder['CsOrder']['extra_mileage_fee'] ?? 0 }}</div>
                    </div>
                    <div class="form-group">
                        <label class="col-lg-6">DIA Fee :</label>
                        <div class="col-lg-6">{{ $csorder['CsOrder']['dia_fee'] ?? 0 }}</div>
                    </div>

                    <div class="form-group heading-divided-updown text-bold">
                        <label class="col-lg-6 ">Total Usage:</label>
                        <div class="col-lg-5">
                            @php $rent = (($csorder['CsOrder']['rent'] ?? 0) + ($csorder['CsOrder']['extra_mileage_fee'] ?? 0) + ($csorder['CsOrder']['dia_fee'] ?? 0) + ($csorder['CsOrder']['initial_fee'] ?? 0)); @endphp
                            {{ $rent }}
                        </div>
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-lg-5">
                            @php $tax = (($csorder['CsOrder']['tax'] ?? 0) + ($csorder['CsOrder']['emf_tax'] ?? 0) + ($csorder['CsOrder']['initial_fee_tax'] ?? 0)); @endphp
                            {{ $tax }}
                        </div>
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-lg-6">{{ $csorder['CsOrder']['insurance_amt'] ?? 0 }}</div>
                    </div>
                    <div class="form-group">
                        <label class="col-lg-6">DIA Ins Add On Fee :</label>
                        <div class="col-lg-6">{{ $csorder['CsOrder']['dia_insu'] ?? 0 }}</div>
                    </div>
                    <div class="form-group heading-divided-updown text-bold">
                        <label class="col-lg-6 ">Total Insurance:</label>
                        <div class="col-lg-6">
                            @php $insufee = sprintf('%0.2f', (($csorder['CsOrder']['insurance_amt'] ?? 0) + ($csorder['CsOrder']['dia_insu'] ?? 0))); @endphp
                            {{ $insufee }}
                        </div>
                    </div>
                    <div class="form-group">
                        ...
                        <div class="col-lg-6">
                            @php $tolls = sprintf('%0.2f', (($csorder['CsOrder']['toll'] ?? 0) + ($csorder['CsOrder']['pending_toll'] ?? 0) + ($csorder['CsOrder']['lateness_fee'] ?? 0) + ($csorder['CsOrder']['damage_fee'] ?? 0) + ($csorder['CsOrder']['uncleanness_fee'] ?? 0))); @endphp
                            {{ $tolls }}
                        </div>
                    </div>
                    <div class="form-group heading-divided-updown text-bold">
                            ...
                            <table width="100%" cellpadding="1" cellspacing="1" border="0" class="table table-responsive">
                                ...
                                <tbody>
                                    @foreach ($payments as $key => $records)
                                        ...
                                        @php $txnamount = 0; $txnrent = 0; $txntax = 0; $txndia_fee = 0; @endphp
                                        @foreach ($records as $payment)
                                            <tr>
                                                ...
                                                <td class="text-center">
                                                    {{ $payment['rent'] }}
                                                    @php $txnrent += $payment['rent']; @endphp
                                                </td>
                                                <td class="text-center">
                                                    {{ $payment['tax'] }}
                                                    @php $txntax += $payment['tax']; @endphp
                                                </td>
                                                <td class="text-center">
                                                    {{ $payment['dia_fee'] }}
                                                    @php $txndia_fee += $payment['dia_fee']; @endphp
                                                </td>
                    ...
                    <div class="form-group heading-divided-updown text-bold">
                        ...
                        <div class="col-lg-6">{{ sprintf('%0.2f', $totalGrandPaid ?? 0) }}</div>
                    </div>
                    <div class="form-group heading-divided-updown text-bold">
                        <label class="col-lg-6 ">Driver Bad Debt :</label>
                        <div class="col-lg-6">{{ sprintf('%0.2f', ($totalDues - ($totalGrandPaid ?? 0))) }}</div>
                    </div>
                    <div class="form-group">
                        <label class="col-lg-6 ">Start Ododmeter :</label>
                        <div class="col-lg-6">{{ $csorder['CsOrder']['start_odometer'] ?? 0 }}</div>
                    </div>
                    <div class="form-group">
                        <label class="col-lg-6 ">End Ododmeter :</label>
                        <div class="col-lg-6">{{ $csorder['CsOrder']['end_odometer'] ?? 0 }}</div>
                    </div>
                    <div class="form-group heading-divided-updown text-bold">
                        <label class="col-lg-6 ">Total Distance :</label>
                        <div class="col-lg-6">{{ ($csorder['CsOrder']['end_odometer'] ?? 0) > 0 ? ($csorder['CsOrder']['end_odometer'] ?? 0) - ($csorder['CsOrder']['start_odometer'] ?? 0) : 'N/A' }}</div>
                    </div>
                    <div class="form-group">
                ...
                <div class="col-lg-6">
                    ...
                    <div class="form-group">
                        <label class="col-lg-6">Write Down Allocation</label>
                        <div class="col-lg-6">
                            @php $writedownallocation = (($calculation['write_down_allocation'] ?? 0) * ($downpaymentPaid ?? 0) / 100); @endphp
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-lg-6">
                            @php $financeallocation = (($calculation['finance_allocation'] ?? 0) * ($downpaymentPaid ?? 0) / 100); @endphp
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-lg-6">
                            @php $maintenanceallocation = (($calculation['maintenance_allocation'] ?? 0) * ($downpaymentPaid ?? 0) / 100); @endphp
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-lg-6">
                            @php $dispositionfee = (($calculation['disposition_fee'] ?? 0) * ($downpaymentPaid ?? 0) / 100); @endphp
                            ...
                            <tbody>
                                @foreach ($extlogs as $extlog)
                                    <tr>
                                        ...
                                        <td style="text-align:center;">
                                            {{ !empty($extlog['OrderExtlog']['created_at']) && strpos($extlog['OrderExtlog']['created_at'], '0000') !== 0 ? \Carbon\Carbon::parse($extlog['OrderExtlog']['created_at'])->timezone(session('default_timezone', config('app.timezone')))->format("m/d/Y h:i A") : "--" }}
                                        </td>
                                    </tr>
                                @endforeach
                            </tbody>
