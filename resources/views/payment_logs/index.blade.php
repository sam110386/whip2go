        ...
        <tbody>
            @forelse ($logs as $log)
                <tr>
                    ...
                    <td>
                        {{ $paymentTypeValue[(int) $log->type] ?? 'Unknown Payment Type' }}
                    </td>
                    <td>
                        {{ $log->amount }}
                    </td>
                    <td>
                        {{ (int) $log->status === 2 ? 'Declined' : 'Success' }}
                    </td>
                    <td>
                        {{ $log->transaction_id }}
                    </td>
                    <td>
                        {{ $log->old_transaction_id }}
                    </td>
                    <td>
                        {{ !empty($log->created) ? \Carbon\Carbon::parse($log->created)->format('Y-m-d h:i A') : '' }}
                    </td>
                    <td>
                        {{ $log->note }}
                    </td>
