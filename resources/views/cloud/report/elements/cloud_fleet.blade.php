        ...
        <tbody>
            @foreach ($lists ?? [] as $list)
                ...
                <tr id="{{ $list['ReportCustomer']['vehicle_id'] ?? '' }}">
                    ...
                    <td style="text-align:center;">
                        {{ ($list['Vehicle']['vehicleCostInclRecon'] ?? 0) + (!empty($list['0']['expenses']) ? $list['0']['expenses'] : 0) + $depriciation - (!empty($list['0']['total_collected']) ? $list['0']['total_collected'] : 0) }}
                    </td>
                </tr>
            @endforeach
        </tbody>
