...
<div class="panel">
    <form action="{{ url('/vehicle_issues/pendingBooking/' . $id) }}" method="POST" name="frmadmin" id="frmadmin" class="form-horizontal">
        @csrf @method('PUT')
        <div class="panel-body">
            ...
            <div class="form-group"><label class="col-lg-2 control-label">&nbsp;</label><div class="col-lg-6"><button type="submit" class="btn btn-primary">{{ !empty($d['id']) ? 'Update' : 'Save' }}</button> <button type="button" class="btn left-margin btn-cancel" onclick="goBack('/vehicle_issues')">Cancel</button></div></div>
        </div>
