...
<script type="text/javascript">
    ...
    jQuery(document).ready(function() {
        ...
        jQuery("#CsVehicleIssueRenterId").select2({
            ...
            ajax: { url: SITE_URL+"vehicle_issues/renterautocomplete", dataType: "json", type: "GET", data: function(params) { return {term: params}; }, processResults: function(data) { return {results: jQuery.map(data, function(item) { return {tag: item.tag, id: item.id}; })}; } },
            initSelection: function(element, callback) { var id = $(element).val(); if (id !== "") { $.ajax(SITE_URL+"vehicle_issues/renterautocomplete", {dataType: "json", type:'POST', data:{id:id}}).done(function(data) { callback(data[0]); }); } }
        });
...
<div class="panel">
    ...
    <div class="panel-body">
        <div class="row">
            ...
            <div class="col-md-6"><div class="form-group"><label class="col-lg-3 control-label">Amount paid for:</label><div class="col-lg-8"><div class="input-group"><span class="input-group-addon"><i class="icon-coin-dollar"></i></span><input type="text" name="CsVehicleIssue[service_paid]" class="form-control digit" value="{{ $d['service_paid'] ?? '' }}"></div></div></div></div>
        </div>
        ...
        <div class="row">
            <div class="col-md-6"><div class="form-group"><label class="col-lg-3 control-label"> Description of Accident: <font class="requiredField">*</font></label><div class="col-lg-8"><textarea name="CsVehicleIssue[accident_description]" class="required form-control">{{ $d['accident_description'] ?? '' }}</textarea><em> What happened?</em></div></div></div>
            <div class="col-md-6"><div class="form-group">
                <div class="col-lg-12"><label class="checkbox-inline checkbox-right"><input type="checkbox" name="CsVehicleIssue[on_way_tolift]" {{ !empty($d['on_way_tolift']) ? 'checked' : '' }}><b>Q. </b>Were you on the way to pick up an Uber/Lyft passenger?</label></div>
                <div class="col-lg-12"><label class="checkbox-inline checkbox-right"><input type="checkbox" name="CsVehicleIssue[have_passenger]" {{ !empty($d['have_passenger']) ? 'checked' : '' }}><b>Q. </b>Did you have a passenger from Uber/Lyft?</label></div>
                <div class="col-lg-12"><label class="checkbox-inline checkbox-right"><input type="checkbox" name="CsVehicleIssue[working_with_delivery]" {{ !empty($d['working_with_delivery']) ? 'checked' : '' }}><b>Q. </b>Were you working with delivery services?</label></div>
                <div class="col-lg-12"><label class="checkbox-inline checkbox-right"><input type="checkbox" name="CsVehicleIssue[orders_from_delivery]" {{ !empty($d['orders_from_delivery']) ? 'checked' : '' }}><b>Q. </b>Had you accepted delivery orders?</label></div>
                <div class="col-lg-12"><label class="checkbox-inline checkbox-right"><input type="checkbox" name="CsVehicleIssue[way_to_drop_off_delivery]" {{ !empty($d['way_to_drop_off_delivery']) ? 'checked' : '' }}><b>Q. </b>On the way to drop off delivery?</label></div>
            </div></div>
        </div>
        <div class="row">
            <div class="col-md-6"><div class="form-group"><label class="col-lg-3 control-label"> Vehicle Damage Details: <font class="requiredField">*</font></label><div class="col-lg-8"><textarea name="CsVehicleIssue[vehicle_damage_description]" class="required form-control">{{ $d['vehicle_damage_description'] ?? '' }}</textarea></div></div></div>
            <div class="col-md-6"><div class="form-group"><label class="col-lg-3 control-label">Vehicle Damage Location:</label><div class="col-lg-8"><input type="text" name="CsVehicleIssue[vehicle_damage_location]" class="form-control" value="{{ $d['vehicle_damage_location'] ?? '' }}"></div></div></div>
        </div>
        <div class="row">
        ...
        @endfor
        <div class="row"><div class="col-md-8"><div class="form-group"><label class="col-lg-2 control-label">Damage Images</label><div class="col-lg-8"><input type="file" class="fileinputajax" multiple name="vehicleimage" data-show-preview="true" data-show-upload="false"><span class="help-block">You can select multiple images.</span></div></div></div></div>
...
foreach($images as $img){ $preview=['caption'=>$img['image'],'filename'=>$img['image'],'key'=>$img['id'],'width'=>'120px','downloadUrl'=>config('app.url').'/img/custom/vehicle_issue/'.$img['image']]; $ext=strtolower(pathinfo($img['image'],PATHINFO_EXTENSION)); if($ext=='pdf') $preview['type']='pdf'; if(in_array($ext,['doc','docx'])) $preview['type']='gdocs'; if(($img['type'] ?? 0)==1){$otherVehicleImages[]=config('app.url').'/img/custom/vehicle_issue/'.$img['image']; $otherVehiclePreviewConfig[]=$preview;}else{$initialPreview[]=config('app.url').'/img/custom/vehicle_issue/'.$img['image']; $initialPreviewConfig[]=$preview;} }
@endphp
<script type="text/javascript">
$(function() {
    var fo = { showDrag: false, showZoom: true, showUpload: false, removeIcon: '<i class="icon-bin"></i>', removeClass: 'btn btn-link btn-xs btn-icon' };
    $(".fileinputajax").fileinput({ showUpload: false, uploadUrl: SITE_URL+"vehicle_issues/saveImage", uploadAsync: true, maxFileCount: 15, deleteUrl: SITE_URL+"vehicle_issues/deleteImage", allowedFileExtensions: ['jpeg','jpg','png','doc','docx','pdf'], initialPreview: {!! json_encode($initialPreview) !!}, overwriteInitial: false, initialPreviewAsData: true, initialPreviewFileType: 'image', initialPreviewConfig: {!! json_encode($initialPreviewConfig) !!}, maxFileSize: 1024, uploadExtraData: function() { return {id: $("#CsVehicleIssueId").val(), type: 0, _token: '{{ csrf_token() }}'}; }, fileActionSettings: fo }).on('fileuploaded', function(e,d,p){$("#"+p+" button.kv-file-remove").attr('data-key',d.response.key);}).on('filebatchuploadcomplete', function(){ goBack('/vehicle_issues'); });
    $(".othervehicleimage").fileinput({ showUpload: false, uploadUrl: SITE_URL+"vehicle_issues/saveImage", uploadAsync: true, maxFileCount: 15, deleteUrl: SITE_URL+"vehicle_issues/deleteImage", allowedFileExtensions: ['jpeg','jpg','png','doc','docx','pdf'], initialPreview: {!! json_encode($otherVehicleImages) !!}, overwriteInitial: false, initialPreviewAsData: true, initialPreviewFileType: 'image', initialPreviewConfig: {!! json_encode($otherVehiclePreviewConfig) !!}, maxFileSize: 1024, uploadExtraData: function() { return {id: $("#CsVehicleIssueId").val(), type: 1, _token: '{{ csrf_token() }}'}; }, fileActionSettings: fo }).on('fileuploaded', function(e,d,p){$("#"+p+" button.kv-file-remove").attr('data-key',d.response.key);}).on('filebatchuploadcomplete', function(){ $('.fileinputajax').fileinput('upload'); });
});
function saveForm() {
    if ($("#frmadmin").valid()) {
        jQuery.blockUI({message: '<h1><img src="'+SITE_URL+'img/select2-spinner.gif" /> Just a moment...</h1>'});
        $.post(SITE_URL+'vehicle_issues/saveaccident', $("#frmadmin").serialize(), function(data) {
