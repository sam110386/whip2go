<!DOCTYPE html>
<html>
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1, maximum-scale=1.0, user-scalable=no" />
    <link rel="stylesheet" href="{{ legacy_asset('css/theme2/bootstrap.css') }}">
    <link rel="stylesheet" href="{{ legacy_asset('css/theme2/core.css') }}">
    <link rel="stylesheet" href="{{ legacy_asset('css/theme2/components.css') }}">
    <link rel="stylesheet" href="{{ legacy_asset('css/theme2/colors.css') }}">
    <link rel="stylesheet" href="{{ legacy_asset('css/driveitawaystyle.css') }}">
    <script async src="https://www.googletagmanager.com/gtag/js?id=UA-133296732-1"></script>
    <script>
        window.dataLayer = window.dataLayer || [];
        function gtag() { dataLayer.push(arguments); }
        gtag('js', new Date());
        gtag('config', 'UA-133296732-1');
    </script>
</head>
<body class="text-center">
    <div class="page-container login-container">
        <div class="page-content">
            <div class="content-wrapper">
                <div class="content">
                    <div class="panel panel-body login-form" id="congratulation">
                        <div class="content-group">&nbsp;</div>
                        <div class="text-center">
                            <div class="col-xs-11">
                                <h4 class="content-group text-info text-uppercase">Congratulations!!! You are done.</h4>
                            </div>
                        </div>
                        <div class="content-group">&nbsp;</div>
                        <div class="content-group">&nbsp;</div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</body>
</html>
